...
public class AltibaseTableColumn extends GenericTableColumn implements DBPNamedObject2 { //, DBSTypedObjectExt4<AltibaseDataType> {
    ...
    @Override
    @Property(viewable = false, editable = true, order = 52)
    ...
    /*
    public AltibaseTableColumn(DBRProgressMonitor monitor, JDBCResultSet dbResult, GenericTableBase table, String columnName, String typeName, int valueType, int sourceType, int ordinalPosition, long columnSize, long charLength, Integer scale, Integer precision, int radix, boolean notNull, String remarks, String defaultValue, boolean autoIncrement, boolean autoGenerated) throws DBException {
        ...
        if (domainTypeName != null) {
            dataType = (AltibaseDataType) table.getDataSource().getLocalDataType(domainTypeName);
        } else {
            dataType = (AltibaseDataType) table.getDataSource().getLocalDataType(typeName);
        }
    }
    ...
    public String getDomainTypeName(DBRProgressMonitor monitor) throws DBException {
        ...
        if (table instanceof AltibaseTableBase) {
            return ((AltibaseTableBase)table).getColumnDomainType(monitor, this);
        }
        return null;
    }
    ...
    @Override
    public void setDataType(AltibaseDataType dataType) {
        this.dataType = dataType;
        this.typeName = dataType.getTypeName();
    }

    public static class FireBirdColumnIncrementValueValidator implements IPropertyValueValidator<AltibaseTableColumn, Object> {

        @Override
        public boolean isValidValue(AltibaseTableColumn column, Object value) throws IllegalArgumentException {
