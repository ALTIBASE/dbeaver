 ...
 */
public class AltibaseExecutionContext extends JDBCExecutionContext implements DBCExecutionContextDefaults<AltibaseCatalog, AltibaseSchema> {
    private static final Log log = Log.getLog(AltibaseExecutionContext.class);

    private String selectedEntityName;

    public AltibaseExecutionContext(@NotNull JDBCRemoteInstance instance, String purpose) {
    ...
    public AltibaseDataSource getDataSource() {
        return (AltibaseDataSource) super.getDataSource();
    }
    ...
    void determineSelectedEntity(DBRProgressMonitor monitor) {
        AltibaseDataSource dataSource = this.getDataSource();
    ...
    void initDefaultsFrom(DBRProgressMonitor monitor, AltibaseExecutionContext context) throws DBCException {
        AltibaseCatalog defaultCatalog = context.getDefaultCatalog();
        ...
        if (defaultCatalog != null && context.supportsCatalogChange()) {
            ...
            AltibaseSchema defaultSchema = context.getDefaultSchema();
            if (defaultSchema != null) {
                entityName = defaultSchema.getName();
            }
        }
        if (entityName != null) {
            AltibaseDataSource dataSource = getDataSource();
    ...
    @Override
    public AltibaseCatalog getDefaultCatalog() {
        if (!CommonUtils.isEmpty(selectedEntityName)) {
            AltibaseDataSource dataSource = getDataSource();
    ...
    public boolean supportsCatalogChange() {
        AltibaseDataSource dataSource = getDataSource();
        if (!(dataSource.getInfo() instanceof AltibaseDataSourceInfo)) {
            return true;
        }
        final AltibaseDataSourceInfo info = (AltibaseDataSourceInfo) dataSource.getInfo();
    ...
    @Override
    public void setDefaultCatalog(DBRProgressMonitor monitor, AltibaseCatalog catalog, AltibaseSchema schema) throws DBCException {
        if (catalog == null) {
            log.debug("Null current catalog");
            return;
        }
        AltibaseDataSource dataSource = getDataSource();
        AltibaseCatalog oldSelectedCatalog = getDefaultCatalog();
        DBCTransactionManager txnManager = null;
        boolean autoCommit = true;
        boolean needToSetAutocommit = false;
        try (JDBCSession session = openSession(monitor, DBCExecutionPurpose.UTIL, "Set active catalog")) {
    ...
    @Override
    public void setDefaultSchema(DBRProgressMonitor monitor, AltibaseSchema schema) throws DBCException {
        if (schema == null) {
            log.debug("Null current schema");
            return;

        }
        AltibaseSchema oldSelectedSchema = getDefaultSchema();
    ...
    public AltibaseObjectContainer getDefaultObject() {
        if (!CommonUtils.isEmpty(selectedEntityName)) {
            AltibaseDataSource dataSource = getDataSource();
            if (dataSource.hasCatalogs()) {
