...
public class AltibaseO2ADependency extends AltibaseO2AObject<DBSObject> implements DBPUniqueObject, DBPImageProvider {
    private final String objectOwnerName;
    private final String objectName;
    private final AltibaseO2AObjectType objectType;

    public AltibaseO2ADependency(DBSObject parent, String objectOwnerName, String objectName, String objectType) {
        super(parent, null, parent.isPersisted());
        this.objectOwnerName = objectOwnerName;
        this.objectName = objectName;
        this.objectType = AltibaseO2AObjectType.getByType(objectType);
    }

    @NotNull
    public static List<AltibaseO2ADependency> readDependencies(@NotNull DBRProgressMonitor monitor, @NotNull DBSObject object, boolean dependents) throws DBException {
        List<AltibaseO2ADependency> dependencies = new ArrayList<>();

        try (JDBCSession session = DBUtils.openMetaSession(monitor, object, "Load object dependencies")) {
            AltibaseO2ADataSource dataSource = (AltibaseO2ADataSource) object.getDataSource();
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                ...
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    while (dbResult.next()) {
                        dependencies.add(new AltibaseO2ADependency(
                            object,
    ...
    public AltibaseO2AObjectType getObjectType() {
        return objectType;
    }

    @Property(viewable = true, order = 3)
    public AltibaseO2ASchema getObjectOwner(DBRProgressMonitor monitor) throws DBException {
