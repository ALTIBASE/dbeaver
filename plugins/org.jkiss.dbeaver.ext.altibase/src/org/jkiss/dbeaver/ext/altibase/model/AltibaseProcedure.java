 ...
 * GenericProcedure
 */
public class AltibaseProcedure extends AbstractProcedure<AltibaseDataSource, AltibaseStructContainer> implements AltibaseScriptObject, DBPUniqueObject
{
    private static final Pattern PATTERN_COL_NAME_NUMERIC = Pattern.compile("\\$?([0-9]+)");

    private String specificName;
    private DBSProcedureType procedureType;
    private List<AltibaseProcedureParameter> columns;
    private String source;
    private AltibaseFunctionResultType functionResultType;

    public AltibaseProcedure(
    ...
    public AltibasePackage getPackage()
    {
        return getContainer() instanceof AltibasePackage ? (AltibasePackage) getContainer() : null;
    }
    ...
    public AltibaseFunctionResultType getFunctionResultType() {
        return functionResultType;
    }

    @Override
    public Collection<AltibaseProcedureParameter> getParameters(DBRProgressMonitor monitor)
        throws DBException
    {
        if (columns == null) {
    ...
    {
        Collection<? extends AltibaseProcedure> procedures = getContainer().getProcedures(monitor, getName());
    ...
    public void addColumn(AltibaseProcedureParameter column)
    {
    ...
    private String makeSignature(DBRProgressMonitor monitor, boolean showParamNames) throws DBException {
        Collection<AltibaseProcedureParameter> parameters = getParameters(monitor);
        if (!CommonUtils.isEmpty(parameters)) {
            StringBuilder paramsSignature = new StringBuilder(64);
            boolean hasParam = false;
            for (AltibaseProcedureParameter param : parameters) {
