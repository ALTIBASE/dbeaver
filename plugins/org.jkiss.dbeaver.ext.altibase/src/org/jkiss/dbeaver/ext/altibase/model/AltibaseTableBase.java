 ...
 * Generic table
 */
public abstract class AltibaseTableBase extends JDBCTable<AltibaseDataSource, AltibaseStructContainer> implements DBPRefreshableObject, DBPSystemObject, DBPScriptObject {
    ...
    @Override
    public AltibaseStructContainer getParentObject() {
    ...
    public AltibaseSchema getSchema() {
        AltibaseStructContainer container = getContainer();
        if (!CommonUtils.isEmpty(tableSchemaName)) {
            if (!(container instanceof AltibaseCatalog)) {
                return getDataSource().getSchema(tableSchemaName);
            } else {
                try {
                    return ((AltibaseCatalog)container).getSchema(new VoidProgressMonitor(), tableSchemaName);
                } catch (Exception e) {
                    log.error(e);
                    return null;
                }
    ...
    @Nullable
    @Override
    public List<? extends AltibaseTableColumn> getAttributes(@NotNull DBRProgressMonitor monitor)
        throws DBException {
    ...
    public void addAttribute(AltibaseTableColumn column) {
        this.getContainer().getTableCache().getChildrenCache(this).cacheObject(column);
    }

    public void removeAttribute(AltibaseTableColumn column) {
        this.getContainer().getTableCache().getChildrenCache(this).removeObject(column, false);
    }

    @Override
    public Collection<? extends AltibaseTableIndex> getIndexes(DBRProgressMonitor monitor)
        throws DBException {
    ...
    @Nullable
    @Override
    public List<AltibaseUniqueKey> getConstraints(@NotNull DBRProgressMonitor monitor)
        throws DBException {
    ...
    public AltibaseUniqueKey getConstraint(@NotNull DBRProgressMonitor monitor, String name) throws DBException {
        if (getDataSource().getInfo().supportsReferentialIntegrity() || getDataSource().getInfo().supportsIndexes()) {
            // ensure all columns are already cached
            getAttributes(monitor);
    ...
    public void addUniqueKey(AltibaseUniqueKey constraint) {
        getContainer().getConstraintKeysCache().cacheObject(constraint);
    }

    @Override
    public Collection<AltibaseTableForeignKey> getReferences(@NotNull DBRProgressMonitor monitor)
        throws DBException {
    ...
    @Override
    public Collection<? extends AltibaseTableForeignKey> getAssociations(@NotNull DBRProgressMonitor monitor)
        throws DBException {
        if (getDataSource().getInfo().supportsReferentialIntegrity()) {
    ...
    public AltibaseTableForeignKey getAssociation(@NotNull DBRProgressMonitor monitor, String name)
        throws DBException {
        if (getDataSource().getInfo().supportsReferentialIntegrity()) {
    ...
    @Association
    @Nullable
    public Collection<AltibaseTableBase> getSubTables() {
        return null;
    }
...
//                }
    ...
    public Long getRowCountFromIndexes(DBRProgressMonitor monitor) {
        try {
            ...
            // Cardinality
            final Collection<? extends AltibaseTableIndex> indexList = getIndexes(monitor);
            if (!CommonUtils.isEmpty(indexList)) {
                for (AltibaseTableIndex index : indexList) {
    ...
    private List<AltibaseTableForeignKey> loadReferences(DBRProgressMonitor monitor)
        throws DBException {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load table relations")) {
            ...
            Map<String, AltibaseTableForeignKey> fkMap = new HashMap<>();
            for (ForeignKeyInfo info : fkInfos) {
                ...
                AltibaseTableBase fkTable = getDataSource().findTable(monitor, info.fkTableCatalog, info.fkTableSchema, info.fkTableName);
                if (fkTable == null) {
                    log.warn("Can't find FK table " + info.fkTableName);
                    continue;
                }
                AltibaseTableColumn pkColumn = this.getAttribute(monitor, info.pkColumnName);
                if (pkColumn == null) {
                    log.warn("Can't find PK column " + info.pkColumnName);
                    continue;
                }
                AltibaseTableColumn fkColumn = fkTable.getAttribute(monitor, info.fkColumnName);
                if (fkColumn == null) {
                ...
                if (pk == null) {
                    Collection<AltibaseUniqueKey> uniqueKeys = this.getConstraints(monitor);
                    if (uniqueKeys != null) {
                ...
                if (pk == null) {
                    ...
                    pk.addColumn(new AltibaseTableConstraintColumn(pk, pkColumn, info.keySeq));
                    // Add this fake constraint to it's owner
                    this.addUniqueKey(pk);
                }

                // Find (or create) FK
                AltibaseTableForeignKey fk;
                ...
                if (fk == null) {
                    ...
                    AltibaseTableForeignKeyColumnTable fkColumnInfo = new AltibaseTableForeignKeyColumnTable(fk, fkColumn, info.keySeq, pkColumn);
    ...
    public List<ForeignKeyInfo> loadReferenceInfoList(@NotNull JDBCSession session, AltibaseMetaObject fkObject) throws SQLException {
    ...
    @Nullable
    @Association
    public List<? extends AltibaseTrigger> getTriggers(@NotNull DBRProgressMonitor monitor) throws DBException {
        if (triggers == null) {
            AltibaseStructContainer parentObject = getParentObject();
    ...
    public List<? extends AltibaseTrigger> getTriggerCache() {
        return triggers;
    }
