...
/**
 * AltibaseProcedureArgument
 */
public class AltibaseO2AProcedureArgument implements DBSProcedureParameter, DBSTypedObject
{
    private final AltibaseO2AProcedureBase procedure;
    private String name;
    private int position;
    private int dataLevel;
    private int sequence;
    private AltibaseO2AParameterMode mode;
    private AltibaseO2ADataType type;
    private AltibaseO2ADataType dataType;
    private String packageTypeName;
    private int dataLength;
    private int dataScale;
    private int dataPrecision;
    private List<AltibaseO2AProcedureArgument> attributes;

    public AltibaseO2AProcedureArgument(
        DBRProgressMonitor monitor,
        AltibaseO2AProcedureBase procedure,
        ResultSet dbResult)
    {
        ...
        if (CommonUtils.isEmpty(this.name)) {
            this.mode =AltibaseO2AParameterMode.RETURN;
        } else {
            this.mode = AltibaseO2AParameterMode.getMode(JDBCUtils.safeGetString(dbResult, "IN_OUT"));
        }
        final String dataType = JDBCUtils.safeGetString(dbResult, "DATA_TYPE");
        this.type = CommonUtils.isEmpty(dataType) ? null : AltibaseO2ADataType.resolveDataType(
            monitor,
        ...
        if (!CommonUtils.isEmpty(typeName) && !CommonUtils.isEmpty(typeOwner) && CommonUtils.isEmpty(this.packageTypeName)) {
            this.dataType = AltibaseO2ADataType.resolveDataType(
                monitor,
                procedure.getDataSource(),
                typeOwner,
                typeName);
    ...
    @Override
    public AltibaseO2AProcedureBase getParentObject()
    {
        return procedure;
    }
    ...
    @Association
    public List<AltibaseO2AProcedureArgument> getAttributes()
    {
        return attributes;
    }

    void addAttribute(AltibaseO2AProcedureArgument attribute)
    {
        if (attributes == null) {
