 ...
 * Altibase utils
 */
public class AltibaseO2AUtils {

    private static final Log log = Log.getLog(AltibaseO2AUtils.class);

    public static String getSysCatalogHint(AltibaseO2ADataSource datasource)
    {
    	return "";
    }
    ...
    public static String getDDL(
        ...
        String objectType,
        AltibaseO2ATableBase object,
        AltibaseO2ADDLFormat ddlFormat,
        Map<String, Object> options) throws DBException
    {
    	/*
        String objectFullName = DBUtils.getObjectFullName(object, DBPEvaluationContext.DDL);

        AltibaseSchema schema = object.getContainer();

        final AltibaseDataSource dataSource = object.getDataSource();
        ...
        try (final JDBCSession session = DBUtils.openMetaSession(monitor, object, "Load source code for " + objectType + " '" + objectFullName + "'")) {
            ...
            if (ddlFormat != AltibaseDDLFormat.COMPACT) {
                // Add object and objects columns info to main DDL
                ddl = addCommentsToDDL(monitor, object, ddl);
            }
            return ddl;
        } catch (SQLException e) {
            if (object instanceof AltibaseTablePhysical) {
                log.error("Error generating Altibase DDL. Generate default.", e);
    ...
    private static String invokeDBMSMetadataGetDependentDDL(JDBCSession session, AltibaseO2ASchema schema, AltibaseO2ATableBase object, DBMSMetaDependentObjectType dependentObjectType) {
        String ddl = "";
        try (JDBCPreparedStatement dbStat = session.prepareStatement(
    ...
    private static String addCommentsToDDL(DBRProgressMonitor monitor, AltibaseO2ATableBase object, String ddl) {
        ...
        try {
            List<AltibaseO2ATableColumn> attributes = object.getAttributes(monitor);
    ...
    public static String normalizeSourceName(AltibaseSourceObject object, boolean body)
    {
        try {
            String source = body ?
    ...
    public static void addSchemaChangeActions(DBCExecutionContext executionContext, List<DBEPersistAction> actions, AltibaseSourceObject object)
    {
        AltibaseO2ASchema schema = object.getSchema();
        if (schema == null) {
            return;
        }
        ...
        AltibaseO2ASchema defaultSchema = ((AltibaseO2AExecutionContext)executionContext).getDefaultSchema();
    ...
    public static String getSysSchemaPrefix(AltibaseO2ADataSource dataSource) {
        boolean useSysView = CommonUtils.toBoolean(dataSource.getContainer().getConnectionConfiguration().getProviderProperty(AltibaseO2AConstants.PROP_METADATA_USE_SYS_SCHEMA));
        if (useSysView) {
            return AltibaseO2AConstants.SCHEMA_SYS + ".";
        } else {
            return "";
        }
    }

    public static String getSource(DBRProgressMonitor monitor, AltibaseSourceObject sourceObject, boolean body, boolean insertCreateReplace) throws DBCException
    {
        ...
        final AltibaseO2ASchema sourceOwner = sourceObject.getSchema();
        if (sourceOwner == null) {
            log.warn("No source owner for object '" + sourceObject.getName() + "'");
            return null;
        }
        monitor.beginTask("Load sources for '" + sourceObject.getName() + "'...", 1);
        String sysViewName = AltibaseO2AConstants.VIEW_DBA_SOURCE;
        if (!sourceObject.getDataSource().isViewAvailable(monitor, AltibaseO2AConstants.SCHEMA_SYS, sysViewName)) {
            sysViewName = AltibaseO2AConstants.VIEW_ALL_SOURCE;
        }
    ...
    public static boolean getObjectStatus(
        DBRProgressMonitor monitor,
        AltibaseStatefulObject object,
        AltibaseO2AObjectType objectType)
        throws DBCException
    {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, object, "Refresh state of " + objectType.getTypeName() + " '" + object.getName() + "'")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT STATUS FROM " + AltibaseO2AUtils.getAdminAllViewPrefix(monitor, object.getDataSource(), "OBJECTS") + " WHERE OBJECT_TYPE=? AND OWNER=? AND OBJECT_NAME=?")) {
    ...
    public static String insertCreateReplace(AltibaseSourceObject object, boolean body, String source) {
    ...
    public static String getAdminAllViewPrefix(DBRProgressMonitor monitor, AltibaseO2ADataSource dataSource, String viewName)
    {
        boolean useDBAView = CommonUtils.toBoolean(dataSource.getContainer().getConnectionConfiguration().getProviderProperty(AltibaseO2AConstants.PROP_ALWAYS_USE_DBA_VIEWS));
        if (useDBAView) {
            String dbaView = "DBA_" + viewName;
            if (dataSource.isViewAvailable(monitor, AltibaseO2AConstants.SCHEMA_SYS, dbaView)) {
                return AltibaseO2AUtils.getSysSchemaPrefix(dataSource) + dbaView;
            }
        }
        return AltibaseO2AUtils.getSysSchemaPrefix(dataSource) + "ALL_" + viewName;
    }
}
