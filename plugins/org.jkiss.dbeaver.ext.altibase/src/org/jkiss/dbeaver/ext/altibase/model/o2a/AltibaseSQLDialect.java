...
public class AltibaseSQLDialect extends JDBCSQLDialect implements SQLDataTypeConverter {
    ...
    protected void loadDataTypesFromDatabase(JDBCDataSource dataSource) {
        ...
        addDataTypes(AltibaseDataType.PREDEFINED_TYPES.keySet());
    }
    ...
    public boolean isDisableScriptEscapeProcessing() {
        return preferenceStore == null || preferenceStore.getBoolean(AltibaseConstants.PREF_DISABLE_SCRIPT_ESCAPE_PROCESSING);
    }
    ...
    public String getColumnTypeModifiers(@NotNull DBPDataSource dataSource, @NotNull DBSTypedObject column, @NotNull String typeName, @NotNull DBPDataKind dataKind) {
        Integer scale;
        switch (typeName) {
            case AltibaseConstants.TYPE_NUMBER:
            case AltibaseConstants.TYPE_DECIMAL:
                ...
                if (precision == 0 || precision > AltibaseConstants.NUMERIC_MAX_PRECISION) {
                    precision = AltibaseConstants.NUMERIC_MAX_PRECISION;
                }
                if (scale != null && precision > 0) {
                    return "(" + precision + ',' + scale + ")";
                }
                break;
            case AltibaseConstants.TYPE_INTERVAL_DAY_SECOND:
                ...
                if (scale < 0 || scale > 9) {
                    scale = AltibaseConstants.INTERVAL_DEFAULT_SECONDS_PRECISION;
                }
                return "(" + scale + ")";
            case AltibaseConstants.TYPE_NAME_BFILE:
            case AltibaseConstants.TYPE_NAME_CFILE:
            case AltibaseConstants.TYPE_CONTENT_POINTER:
            case AltibaseConstants.TYPE_LONG:
            case AltibaseConstants.TYPE_LONG_RAW:
            case AltibaseConstants.TYPE_OCTET:
            case AltibaseConstants.TYPE_INTERVAL_YEAR_MONTH:
    ...
    public String convertExternalDataType(@NotNull SQLDialect sourceDialect, @NotNull DBSTypedObject sourceTypedObject, @Nullable DBPDataTypeProvider targetTypeProvider) {
        ...
        switch (externalTypeName) {
            case "VARCHAR":
                ...
                localDataType = AltibaseConstants.TYPE_NAME_VARCHAR;
            ...
            case "XML":
            case "XMLTYPE":
                localDataType = AltibaseConstants.TYPE_FQ_XML;
                break;
            case "JSON":
            case "JSONB":
                localDataType = "JSON";
            ...
            case "GEOGRAPHY":
            case "SDO_GEOMETRY":
                localDataType = AltibaseConstants.TYPE_FQ_GEOMETRY;
                break;
            case "NUMERIC":
                localDataType = AltibaseConstants.TYPE_NUMBER;
