 ...
 * Altibase sequence
 */
public class AltibaseO2ASequence extends AltibaseO2ASchemaObject implements DBSSequence, DBPScriptObject {
    ...
    public AltibaseO2ASequence(AltibaseO2ASchema schema, String name) {
    ...
    public AltibaseO2ASequence(AltibaseO2ASchema schema, ResultSet dbResult)
    {
        super(schema, JDBCUtils.safeGetString(dbResult, "TABBLE_NAME"), true);
        
        this.currentValue = JDBCUtils.safeGetBigDecimal(dbResult, "CURRENT_SEQ");
        this.startValue = JDBCUtils.safeGetBigDecimal(dbResult, "START_SEQ");
        this.minValue = JDBCUtils.safeGetBigDecimal(dbResult, "MIN_SEQ");
        this.maxValue = JDBCUtils.safeGetBigDecimal(dbResult, "MAX_SEQ");
        this.incrementBy = JDBCUtils.safeGetLong(dbResult, "INCREMENT_SEQ");
        this.cacheSize = JDBCUtils.safeGetLong(dbResult, "CACHE_SIZE");
        this.lastValue = JDBCUtils.safeGetBigDecimal(dbResult, "LAST_NUMBER");
        this.flagCycle = JDBCUtils.safeGetString(dbResult, "IS_CYCLE").equalsIgnoreCase(AltibaseO2AConstants.YES)?true:false;
    }
