 ...
 * AltibaseView
 */
public class AltibaseO2AView extends AltibaseO2ATableBase implements AltibaseSourceObject, DBSView
{
    private static final Log log = Log.getLog(AltibaseO2AView.class);
    ...
    public class AdditionalInfo extends TableAdditionalInfo {
        ...
        private AltibaseO2AView superView;
        ...
        public Object getType(DBRProgressMonitor monitor) throws DBException
        {
            ...
            AltibaseO2ASchema owner = getDataSource().getSchema(monitor, typeOwner);
        ...
        public AltibaseO2AView getSuperView() { return superView; }
        public void setSuperView(AltibaseO2AView superView) { this.superView = superView; }
    }
    ...
    private AltibaseO2ADDLFormat currentDDLFormat;

    public AltibaseO2AView(AltibaseO2ASchema schema, String name)
    {
        super(schema, name, false);
    }

    public AltibaseO2AView(
    ...
    {
        return AltibaseO2ASourceType.VIEW;
    }
    ...
    {
        if (viewText == null) {
             currentDDLFormat = AltibaseO2ADDLFormat.getCurrentFormat(getDataSource());
        }
        AltibaseO2ADDLFormat newFormat = AltibaseO2ADDLFormat.FULL;
        boolean isFormatInOptions = !CommonUtils.isEmpty(options) && options.containsKey(AltibaseO2AConstants.PREF_KEY_DDL_FORMAT);
        if (isFormatInOptions) {
            newFormat = (AltibaseO2ADDLFormat) options.get(AltibaseO2AConstants.PREF_KEY_DDL_FORMAT);
        }

        if (viewText == null || (currentDDLFormat != newFormat && isPersisted())) {
            try {
                if (viewText == null || !isFormatInOptions) {
                    viewText = AltibaseO2AUtils.getDDL(monitor, getTableTypeName(), this, currentDDLFormat, options);
                } else {
                    viewText = AltibaseO2AUtils.getDDL(monitor, getTableTypeName(), this, newFormat, options);
        ...
        if (CommonUtils.isEmpty(viewText)) {
            ...
            if (CommonUtils.isEmpty(viewSourceText)) {
                return "-- Altibase view definition is not available";
            }
    ...
    {
        this.valid = AltibaseO2AUtils.getObjectStatus(monitor, this, AltibaseO2AObjectType.VIEW);
    }
    ...
    private void loadAdditionalInfo(DBRProgressMonitor monitor) throws DBException
    {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load table status")) {
            boolean isAltibase9 = getDataSource().isAtLeastV9();
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT TEXT,TYPE_TEXT,OID_TEXT,VIEW_TYPE_OWNER,VIEW_TYPE" + (isAltibase9 ? ",SUPERVIEW_NAME" : "") + "\n" +
                    "FROM  WHERE OWNER=? AND VIEW_NAME=?")) {
        ...
        if (viewDefinitionText != null) {
            ...
            Collection<AltibaseO2ATableColumn> attributes = getAttributes(monitor);
            if (attributes != null) {
                paramsList.append("\n(");
                boolean first = true;
                for (AltibaseO2ATableColumn column : attributes) {
