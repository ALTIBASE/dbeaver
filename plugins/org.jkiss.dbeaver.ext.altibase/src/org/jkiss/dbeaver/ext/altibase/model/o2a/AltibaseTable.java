 ...
 * AltibaseTable
 */
public class AltibaseTable extends AltibaseTablePhysical implements DBPScriptObject, DBDPseudoAttributeContainer,
    ...
    private AltibaseDataType tableType;
    ...
    public AltibaseTable(AltibaseSchema schema, String name)
    {
        super(schema, name);
    }

    public AltibaseTable(
        ...
        ResultSet dbResult)
    {
        ...
        if (!CommonUtils.isEmpty(typeOwner)) {
            tableType = AltibaseDataType.resolveDataType(
                monitor,
    ...
    public AltibaseDataType getTableType()
    {
        return tableType;
    }
    ...
    @Override
    public AltibaseTableColumn getAttribute(@NotNull DBRProgressMonitor monitor, @NotNull String attributeName) throws DBException {
/*
...
*/
    ...
    @Nullable
    private AltibaseTableColumn getXMLColumn(DBRProgressMonitor monitor) throws DBException {
    ...
    public Collection<AltibaseTableForeignKey> getReferences(@NotNull DBRProgressMonitor monitor)
        throws DBException
    {
        List<AltibaseTableForeignKey> refs = new ArrayList<>();
        // This is dummy implementation
        // Get references from this schema only
        final Collection<AltibaseTableForeignKey> allForeignKeys =
            getContainer().foreignKeyCache.getObjects(monitor, getContainer(), null);
        for (AltibaseTableForeignKey constraint : allForeignKeys) {
    ...
    @Override
    @Association
    public Collection<AltibaseTableForeignKey> getAssociations(@NotNull DBRProgressMonitor monitor)
        throws DBException
    {
    ...
    protected void appendSelectSource(DBRProgressMonitor monitor, StringBuilder query, String tableAlias, DBDPseudoAttribute rowIdAttribute) {
        if (tableType != null && tableType.getName().equals(AltibaseConstants.TYPE_NAME_XML)) {
            try {
                AltibaseTableColumn xmlColumn = getXMLColumn(monitor);
                if (xmlColumn != null) {
    ...
    public String getObjectDefinitionText(DBRProgressMonitor monitor, Map<String, Object> options) throws DBException {
        return getDDL(monitor, AltibaseDDLFormat.getCurrentFormat(getDataSource()), options);
    }
    ...
    public void enableReferentialIntegrity(@NotNull DBRProgressMonitor monitor, boolean enable) throws DBException {
        Collection<AltibaseTableForeignKey> foreignKeys = getAssociations(monitor);
