...
/**
 * Altibase materialized view
 */
public class AltibaseO2AMaterializedView extends AltibaseO2ATableBase implements AltibaseSourceObject, DBSObjectLazy<AltibaseO2ADataSource>
{
    private static final Log log = Log.getLog(AltibaseO2AMaterializedView.class);
    ...
    public AltibaseO2AMaterializedView(AltibaseO2ASchema schema, String name)
    {
        super(schema, name, false);
    }

    public AltibaseO2AMaterializedView(
    ...
    {
        return AltibaseO2ASourceType.MATERIALIZED_VIEW;
    }
        ...
        if (query == null || currentDDLFormat != newFormat && isPersisted()) {
            try {
                if (query == null || !isFormatInOptions) {
                    query = AltibaseO2AUtils.getDDL(monitor, getTableTypeName(), this, currentDDLFormat, options);
                } else {
                    query = AltibaseO2AUtils.getDDL(monitor, getTableTypeName(), this, newFormat, options);
    ...
    public void setCurrentDDLFormat(AltibaseO2ADDLFormat currentDDLFormat) {
    ...
    private void loadAdditionalInfo(DBRProgressMonitor monitor) throws DBCException
    {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load table status")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT * FROM  WHERE OWNER=? AND MVIEW_NAME=?")) {
    ...
    public DBEPersistAction[] getCompileActions(DBRProgressMonitor monitor)
    {
        return new DBEPersistAction[] {
            new AltibaseO2AObjectPersistAction(
                AltibaseO2AObjectType.MATERIALIZED_VIEW,
    ...
    {
        this.valid = AltibaseO2AUtils.getObjectStatus(monitor, this, AltibaseO2AObjectType.MATERIALIZED_VIEW);
    }
