 ...
 * GenericDataSource
 */
public class AltibaseDataSource extends JDBCDataSource implements DBPTermProvider, IAdaptable, AltibaseStructContainer, DBCQueryPlanner {
    private static final Log log = Log.getLog(AltibaseDataSource.class);

    private final TableTypeCache tableTypeCache;
    private final JDBCBasicDataTypeCache<AltibaseStructContainer, ? extends JDBCDataType> dataTypeCache;
    private List<AltibaseCatalog> catalogs;
    private SimpleObjectCache<AltibaseStructContainer, AltibaseSchema> schemas;
    private final AltibaseMetaModel metaModel;
    private AltibaseObjectContainer structureContainer;
    ...
    public AltibaseDataSource(@NotNull DBRProgressMonitor monitor, @NotNull DBPDataSourceContainer container, @NotNull AltibaseMetaModel metaModel, @NotNull SQLDialect dialect)
        throws DBException {
    ...
    @ForTest
    public AltibaseDataSource(@NotNull DBRProgressMonitor monitor, @NotNull AltibaseMetaModel metaModel, @NotNull DBPDataSourceContainer container, @NotNull SQLDialect dialect)
            throws DBException {
        super(monitor, container, dialect, false);
        this.metaModel = metaModel;
        this.dataTypeCache = metaModel.createDataTypeCache(this);
    ...
    protected boolean isPopulateClientAppName() {
        return getClass() != AltibaseDataSource.class;
    }
    ...
    protected void initializeContextState(@NotNull DBRProgressMonitor monitor, @NotNull JDBCExecutionContext context, JDBCExecutionContext initFrom) throws DBException {
        ...
        if (initFrom != null) {
            AltibaseExecutionContext metaContext = (AltibaseExecutionContext) initFrom;
            ((AltibaseExecutionContext) context).initDefaultsFrom(monitor, metaContext);
        } else {
    ...
    protected DBPDataSourceInfo createDataSourceInfo(DBRProgressMonitor monitor, @NotNull JDBCDatabaseMetaData metaData) {
        final AltibaseDataSourceInfo info = new AltibaseDataSourceInfo(getContainer().getDriver(), metaData);
    ...
    @Association
    public Collection<AltibaseTableType> getTableTypes(DBRProgressMonitor monitor)
        throws DBException {
        return tableTypeCache.getAllObjects(monitor, this);
    }

    public List<AltibaseCatalog> getCatalogs() {
        return catalogs;
    }

    public AltibaseCatalog getCatalog(String name) {
        return DBUtils.findObject(getCatalogs(), name);
    }

    public final Collection<AltibaseCatalog> getCatalogList() {
...
//        }
    ...
    public final Collection<AltibaseSchema> getSchemaList() {
        if (getDataSource().isMergeEntities()) {
            return null;
        }
        return getSchemas();
    ...
    @Association
    public List<AltibaseSchema> getSchemas() {
        return schemas == null ? null : schemas.getCachedObjects();
    }

    public AltibaseSchema getSchema(String name) {
    ...
    @Override
    public AltibaseStructContainer getObject() {
        return this;
    }

    @Override
    public AltibaseCatalog getCatalog() {
        return null;
    }
    ...
    @Override
    public AltibaseObjectContainer.GenericSequenceCache getSequenceCache() {
        return structureContainer.getSequenceCache();
    }

    @Override
    public AltibaseObjectContainer.GenericSynonymCache getSynonymCache() {
        return structureContainer.getSynonymCache();
    }

    @Override
    public List<? extends AltibaseView> getViews(DBRProgressMonitor monitor) throws DBException {
        return structureContainer == null ? null : structureContainer.getViews(monitor);
    }

    @Override
    public List<? extends AltibaseTable> getPhysicalTables(DBRProgressMonitor monitor) throws DBException {
        return structureContainer == null ? null : structureContainer.getPhysicalTables(monitor);
    }

    @Override
    public List<? extends AltibaseTableBase> getTables(DBRProgressMonitor monitor)
        throws DBException {
        return structureContainer == null ? null : structureContainer.getTables(monitor);
    }

    @Override
    public AltibaseTableBase getTable(DBRProgressMonitor monitor, String name)
        throws DBException {
    ...
    @Override
    public Collection<? extends AltibaseProcedure> getProcedures(DBRProgressMonitor monitor)
        throws DBException {
        return structureContainer == null ? null : structureContainer.getProcedures(monitor);
    }

    @Override
    public Collection<? extends AltibaseProcedure> getProceduresOnly(DBRProgressMonitor monitor) throws DBException {
        return structureContainer == null ? null : structureContainer.getProceduresOnly(monitor);
    }

    @Override
    public AltibaseProcedure getProcedure(DBRProgressMonitor monitor, String uniqueName) throws DBException {
        return structureContainer == null ? null : structureContainer.getProcedure(monitor, uniqueName);
    }

    @Override
    public Collection<AltibaseProcedure> getProcedures(DBRProgressMonitor monitor, String name)
        throws DBException {
        return structureContainer == null ? null : structureContainer.getProcedures(monitor, name);
    }

    @Override
    public Collection<? extends AltibaseProcedure> getFunctionsOnly(DBRProgressMonitor monitor) throws DBException {
    ...
    @Override
    public Collection<? extends AltibaseSynonym> getSynonyms(DBRProgressMonitor monitor) throws DBException {
        return structureContainer == null ? null : structureContainer.getSynonyms(monitor);
    }

    @Override
    public Collection<? extends AltibaseTrigger> getTriggers(DBRProgressMonitor monitor) throws DBException {
        return structureContainer == null ? null : structureContainer.getTriggers(monitor);
    }

    @Override
    public Collection<? extends AltibaseTrigger> getTableTriggers(DBRProgressMonitor monitor) throws DBException {
    ...
    public void initialize(@NotNull DBRProgressMonitor monitor) throws DBException {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Read generic metadata")) {
            ...
            if (!omitCatalog) {
                ...
                monitor.worked(1);
                final AltibaseMetaObject catalogObject = getMetaObject(GenericConstants.OBJECT_CATALOG);
                final DBSObjectFilter catalogFilters = getContainer().getObjectFilter(AltibaseCatalog.class, null, false);
                ...
                if (!catalogNames.isEmpty() || catalogsFiltered) {
                    ...
                    for (String catalogName : catalogNames) {
                        AltibaseCatalog catalog = metaModel.createCatalogImpl(this, catalogName);
            ...
            if (CommonUtils.isEmpty(catalogs) && !catalogsFiltered) {
                ...
                try {
                    List<AltibaseSchema> tmpSchemas = metaModel.loadSchemas(session, this, null);
                    if (tmpSchemas != null) {
    ...
    public List<String> getCatalogsNames(@NotNull DBRProgressMonitor monitor, @NotNull JDBCDatabaseMetaData metaData, AltibaseMetaObject catalogObject, @Nullable DBSObjectFilter catalogFilters) throws DBException {
        final List<String> catalogNames = new ArrayList<>();
        try {
            try (JDBCResultSet dbResult = metaData.getCatalogs()) {
                int totalCatalogs = 0;
                while (dbResult.next()) {
                    String catalogName = AltibaseUtils.safeGetString(catalogObject, dbResult, JDBCConstants.TABLE_CAT);
                    if (CommonUtils.isEmpty(catalogName)) {
                        // Some drivers uses TABLE_QUALIFIER instead of catalog
                        catalogName = AltibaseUtils.safeGetStringTrimmed(catalogObject, dbResult, JDBCConstants.TABLE_QUALIFIER);
                        if (CommonUtils.isEmpty(catalogName)) {
                            continue;
                        }
                    }
    ...
    public boolean isOmitCatalog() {
        //return CommonUtils.getBoolean(getContainer().getDriver().getDriverParameter(AltibaseConstants.PARAM_OMIT_CATALOG), true);
    	return true;
    }
    ...
    AltibaseTableBase findTable(@NotNull DBRProgressMonitor monitor, String catalogName, String schemaName, String tableName)
        throws DBException {
        AltibaseObjectContainer container = null;
        ...
        if (!CommonUtils.isEmpty(schemaName)) {
            if (container != null) {
                container = ((AltibaseCatalog) container).getSchema(monitor, schemaName);
    ...
    public void cacheStructure(@NotNull DBRProgressMonitor monitor, int scope) throws DBException {
        if (!CommonUtils.isEmpty(catalogs)) {
            for (AltibaseCatalog catalog : catalogs) catalog.cacheStructure(monitor, scope);
        } else if (!schemas.isEmpty()) {
            for (AltibaseSchema schema : schemas.getCachedObjects()) schema.cacheStructure(monitor, scope);
        } else if (structureContainer != null) {
    ...
    private boolean isChild(DBSObject object) throws DBException {
        if (object instanceof AltibaseCatalog) {
            return !CommonUtils.isEmpty(catalogs) && catalogs.contains(object);
        } else if (object instanceof AltibaseSchema) {
    ...
    protected JDBCBasicDataTypeCache<AltibaseStructContainer, ? extends JDBCDataType> getDataTypeCache() {
    ...
    AltibaseCatalog getDefaultCatalog() {
        return null;
    }

    AltibaseSchema getDefaultSchema() {
        if (schemas != null) {
            for (AltibaseSchema schema : schemas.getCachedObjects()) {
                if (schema.isVirtual()) {
                    return schema;
                }
            }
    ...
    private class TableTypeCache extends JDBCObjectCache<AltibaseDataSource, AltibaseTableType> {
        @NotNull
        @Override
        ...
        @Override
        protected AltibaseTableType fetchObject(@NotNull JDBCSession session, @NotNull AltibaseDataSource owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
            return new AltibaseTableType(
                AltibaseDataSource.this,
                AltibaseUtils.safeGetString(
    ...
    private class DataSourceObjectContainer extends AltibaseObjectContainer {
        private DataSourceObjectContainer() {
            super(AltibaseDataSource.this);
        }
        ...
        @Override
        public AltibaseStructContainer getObject() {
            return AltibaseDataSource.this;
        }
        ...
        public String getName() {
            return AltibaseDataSource.this.getName();
        }

        @Nullable
        @Override
        public String getDescription() {
            return AltibaseDataSource.this.getDescription();
        }

        @Override
        public DBSObject getParentObject() {
            return AltibaseDataSource.this.getParentObject();
        }
    }
