 ...
 */
public class AltibaseProcedureArgument implements DBSProcedureParameter, DBSTypedObject
{
    private final AltibaseProcedureBase procedure;
    private String name;
    private int position;
    private int dataLevel;
    private int sequence;
    private AltibaseParameterMode mode;
    private AltibaseDataType type;
    private AltibaseDataType dataType;
    private String packageTypeName;
    private int dataLength;
    private int dataScale;
    private int dataPrecision;
    private List<AltibaseProcedureArgument> attributes;

    public AltibaseProcedureArgument(
        DBRProgressMonitor monitor,
        AltibaseProcedureBase procedure,
        ResultSet dbResult)
    {
        ...
        if (CommonUtils.isEmpty(this.name)) {
            this.mode =AltibaseParameterMode.RETURN;
        } else {
            this.mode = AltibaseParameterMode.getMode(JDBCUtils.safeGetString(dbResult, "IN_OUT"));
        }
        final String dataType = JDBCUtils.safeGetString(dbResult, "DATA_TYPE");
        this.type = CommonUtils.isEmpty(dataType) ? null : AltibaseDataType.resolveDataType(
            monitor,
        ...
        if (!CommonUtils.isEmpty(typeName) && !CommonUtils.isEmpty(typeOwner) && CommonUtils.isEmpty(this.packageTypeName)) {
            this.dataType = AltibaseDataType.resolveDataType(
                monitor,
                procedure.getDataSource(),
                typeOwner,
                typeName);
    ...
    @Override
    public AltibaseProcedureBase getParentObject()
    {
        return procedure;
    }
    ...
    @Association
    public List<AltibaseProcedureArgument> getAttributes()
    {
        return attributes;
    }

    void addAttribute(AltibaseProcedureArgument attribute)
    {
        if (attributes == null) {
