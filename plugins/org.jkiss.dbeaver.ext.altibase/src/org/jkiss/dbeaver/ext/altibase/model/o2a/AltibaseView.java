 ...
 * AltibaseView
 */
public class AltibaseView extends AltibaseTableBase implements AltibaseSourceObject, DBSView
{
    private static final Log log = Log.getLog(AltibaseView.class);
    ...
    public class AdditionalInfo extends TableAdditionalInfo {
        ...
        private AltibaseView superView;
        ...
        public Object getType(DBRProgressMonitor monitor) throws DBException
        {
            ...
            AltibaseSchema owner = getDataSource().getSchema(monitor, typeOwner);
        ...
        public AltibaseView getSuperView() { return superView; }
        public void setSuperView(AltibaseView superView) { this.superView = superView; }
    }
    ...
    private AltibaseDDLFormat currentDDLFormat;

    public AltibaseView(AltibaseSchema schema, String name)
    {
    ...
    {
        return AltibaseSourceType.VIEW;
    }
    ...
    {
        if (viewText == null) {
             currentDDLFormat = AltibaseDDLFormat.getCurrentFormat(getDataSource());
        }
        AltibaseDDLFormat newFormat = AltibaseDDLFormat.FULL;
        boolean isFormatInOptions = !CommonUtils.isEmpty(options) && options.containsKey(AltibaseConstants.PREF_KEY_DDL_FORMAT);
        if (isFormatInOptions) {
            newFormat = (AltibaseDDLFormat) options.get(AltibaseConstants.PREF_KEY_DDL_FORMAT);
        }

        if (viewText == null || (currentDDLFormat != newFormat && isPersisted())) {
            try {
                if (viewText == null || !isFormatInOptions) {
                    viewText = AltibaseUtils.getDDL(monitor, getTableTypeName(), this, currentDDLFormat, options);
                } else {
                    viewText = AltibaseUtils.getDDL(monitor, getTableTypeName(), this, newFormat, options);
    ...
    {
        this.valid = AltibaseUtils.getObjectStatus(monitor, this, AltibaseObjectType.VIEW);
    }
    ...
    private void loadAdditionalInfo(DBRProgressMonitor monitor) throws DBException
    {
        ...
        if (viewDefinitionText != null) {
            ...
            Collection<AltibaseTableColumn> attributes = getAttributes(monitor);
            if (attributes != null) {
                paramsList.append("\n(");
                boolean first = true;
                for (AltibaseTableColumn column : attributes) {
