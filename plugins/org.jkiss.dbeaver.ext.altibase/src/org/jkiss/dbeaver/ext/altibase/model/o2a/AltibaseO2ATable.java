 ...
 * AltibaseTable
 */
public class AltibaseO2ATable extends AltibaseO2ATablePhysical implements DBPScriptObject, DBDPseudoAttributeContainer,
        DBPObjectStatistics, DBPImageProvider, DBPReferentialIntegrityController, DBPScriptObjectExt2 {
    private static final Log log = Log.getLog(AltibaseO2ATable.class);
    ...
    private AltibaseO2ADataType tableType;
    ...
    public AltibaseO2ATable(AltibaseO2ASchema schema, String name)
    {
        super(schema, name);
    }

    public AltibaseO2ATable(
        ...
        ResultSet dbResult)
    {
        ...
        if (!CommonUtils.isEmpty(typeOwner)) {
            tableType = AltibaseO2ADataType.resolveDataType(
                monitor,
    ...
    public AltibaseO2ADataType getTableType()
    {
        return tableType;
    }
    ...
    @Override
    public AltibaseO2ATableColumn getAttribute(@NotNull DBRProgressMonitor monitor, @NotNull String attributeName) throws DBException {
/*
        // Fake XML attribute handle
        if (tableType != null && tableType.getName().equals(AltibaseConstants.TYPE_NAME_XML) && AltibaseConstants.XML_COLUMN_NAME.equals(attributeName)) {
            AltibaseTableColumn col = getXMLColumn(monitor);
...
*/
    ...
    @Nullable
    private AltibaseO2ATableColumn getXMLColumn(DBRProgressMonitor monitor) throws DBException {
    ...
    @Override
    public Collection<AltibaseO2ATableForeignKey> getReferences(@NotNull DBRProgressMonitor monitor)
        throws DBException
    {
        List<AltibaseO2ATableForeignKey> refs = new ArrayList<>();
        // This is dummy implementation
        // Get references from this schema only
        final Collection<AltibaseO2ATableForeignKey> allForeignKeys =
            getContainer().foreignKeyCache.getObjects(monitor, getContainer(), null);
        for (AltibaseO2ATableForeignKey constraint : allForeignKeys) {
    ...
    @Override
    @Association
    public Collection<AltibaseO2ATableForeignKey> getAssociations(@NotNull DBRProgressMonitor monitor)
        throws DBException
    {
    ...
    protected void appendSelectSource(DBRProgressMonitor monitor, StringBuilder query, String tableAlias, DBDPseudoAttribute rowIdAttribute) {
        if (tableType != null && tableType.getName().equals(AltibaseO2AConstants.TYPE_NAME_XML)) {
            try {
                AltibaseO2ATableColumn xmlColumn = getXMLColumn(monitor);
                if (xmlColumn != null) {
    ...
    public String getObjectDefinitionText(DBRProgressMonitor monitor, Map<String, Object> options) throws DBException {
        return getDDL(monitor, AltibaseO2ADDLFormat.getCurrentFormat(getDataSource()), options);
    }
    ...
    private void loadAdditionalInfo(DBRProgressMonitor monitor) throws DBException
    {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load table status")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT * FROM SYSTEM_.SYSTABLES_ WHERE OWNER=? AND TABLE_NAME=?")) {
    ...
    public void enableReferentialIntegrity(@NotNull DBRProgressMonitor monitor, boolean enable) throws DBException {
        Collection<AltibaseO2ATableForeignKey> foreignKeys = getAssociations(monitor);
