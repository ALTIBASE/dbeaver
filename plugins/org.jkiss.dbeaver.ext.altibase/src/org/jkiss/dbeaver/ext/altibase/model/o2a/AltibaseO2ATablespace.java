 ...
 * Altibase tablespace
 */
public class AltibaseO2ATablespace extends AltibaseO2AGlobalObject implements DBPRefreshableObject, DBPObjectStatistics
{
    ...
    protected AltibaseO2ATablespace(AltibaseO2ADataSource dataSource, ResultSet dbResult)
    {
    ...
    @Association
    public Collection<AltibaseO2ADataFile> getFiles(DBRProgressMonitor monitor) throws DBException
    {
        return fileCache.getAllObjects(monitor, this);
    }

    public AltibaseO2ADataFile getFile(DBRProgressMonitor monitor, long relativeFileNo) throws DBException
    {
        for (AltibaseO2ADataFile file : fileCache.getAllObjects(monitor, this)) {
    ...
    private void loadSizes(DBRProgressMonitor monitor) throws DBException {
        try (final JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load tablespace '" + getName() + "' statistics")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement("SELECT * FROM\n" +
                "(SELECT SUM(F.BYTES) AVAILABLE_SPACE FROM " + AltibaseO2AUtils.getSysSchemaPrefix(getDataSource()) + "DBA_DATA_FILES F WHERE F.TABLESPACE_NAME=?) XDF,\n" +
                "(SELECT SUM(S.BYTES) USED_SPACE FROM " + AltibaseO2AUtils.getSysSchemaPrefix(getDataSource()) + "DBA_SEGMENTS S WHERE S.TABLESPACE_NAME=?) XS")) {
    ...
    static class FileCache extends JDBCObjectCache<AltibaseO2ATablespace, AltibaseO2ADataFile> {
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull AltibaseO2ATablespace owner) throws SQLException
        {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT * FROM " + AltibaseO2AUtils.getSysSchemaPrefix(owner.getDataSource()) + "DBA_" +
        ...
        @Override
        protected AltibaseO2ADataFile fetchObject(@NotNull JDBCSession session, @NotNull AltibaseO2ATablespace owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException
        {
            return new AltibaseO2ADataFile(owner, resultSet, owner.getContents() == Contents.TEMPORARY);
        }
    }

    static Object resolveTablespaceReference(DBRProgressMonitor monitor, DBSObjectLazy<AltibaseO2ADataSource> referrer, @Nullable Object propertyId) throws DBException
    {
        final AltibaseO2ADataSource dataSource = referrer.getDataSource();
        if (!dataSource.isAdmin()) {
            return referrer.getLazyReference(propertyId);
        } else {
            return AltibaseO2AUtils.resolveLazyReference(monitor, dataSource, dataSource.tablespaceCache, referrer, propertyId);
        }
    }
    
    public static class TablespaceReferenceValidator implements IPropertyCacheValidator<DBSObjectLazy<AltibaseO2ADataSource>> {
        @Override
        public boolean isPropertyCached(DBSObjectLazy<AltibaseO2ADataSource> object, Object propertyId)
        {
            return
                object.getLazyReference(propertyId) instanceof AltibaseO2ATablespace ||
