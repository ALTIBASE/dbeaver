 ...
 */
public abstract class AltibaseObjectContainer implements AltibaseStructContainer, DBPRefreshableObject {
    private static final Log log = Log.getLog(AltibaseObjectContainer.class);
    ...
    private List<AltibasePackage> packages;
    protected List<AltibaseProcedure> procedures;
    protected List<? extends AltibaseSequence> sequences;
    protected List<? extends AltibaseSynonym> synonyms;

    protected AltibaseObjectContainer(@NotNull AltibaseDataSource dataSource) {
    ...
    public TableCache createTableCache(AltibaseDataSource datasource) {
    ...
    public List<? extends AltibaseView> getViews(DBRProgressMonitor monitor) throws DBException {
        List<? extends AltibaseTableBase> tables = getTables(monitor);
        if (tables != null) {
            List<AltibaseView> filtered = new ArrayList<>();
            for (AltibaseTableBase table : tables) {
    ...
    private void cacheIndexes(DBRProgressMonitor monitor, boolean readFromTables)
        ...
        synchronized (indexCache) {
            if (!indexCache.isFullyCached()) {
                ...
                if (readFromTables && newIndexCache.isEmpty()) {
                    ...
                    try {
                        for (AltibaseTableBase table : tables) {
                            ...
                            Collection<? extends AltibaseTableIndex> tableIndexes = table.getIndexes(monitor);
                ...
                for (AltibaseTableIndex oldIndex : oldCache) {
                    if (!oldIndex.isPersisted()) {
                        newIndexCache.add(oldIndex);
                    } else {
                        // Check for the dups
                        for (int i = 0; i < newIndexCache.size(); i++) {
                            AltibaseTableIndex newIndex = newIndexCache.get(i);
    ...
    public void cacheStructure(@NotNull DBRProgressMonitor monitor, int scope)
        ...
        if ((scope & STRUCT_ASSOCIATIONS) != 0 && dataSource.supportsStructCache()) {
            ...
            try {
                ...
                Collection<AltibaseUniqueKey> objects = constraintKeysCache.getObjects(monitor, this, null);
            ...
            if (dataSource.getInfo().supportsReferentialIntegrity()) {
                ...
                try {
                    ...
                    Collection<AltibaseTableForeignKey> foreignKeys = foreignKeysCache.getObjects(monitor, this, null);
    ...
    public AltibasePackage getPackage(DBRProgressMonitor monitor, String name)
        throws DBException {
        return DBUtils.findObject(getPackages(monitor), name);
    }

    public List<AltibaseProcedure> getProcedureCache() {
        return procedures;
    }

    @Override
    public List<AltibaseProcedure> getProcedures(DBRProgressMonitor monitor)
        throws DBException {
        if (procedures == null) {
            loadProcedures(monitor);
        }
    ...
    public AltibaseProcedure getProcedure(DBRProgressMonitor monitor, String uniqueName) throws DBException {
        for (AltibaseProcedure procedure : CommonUtils.safeCollection(getProcedures(monitor))) {
    ...
    @Override
    public List<AltibaseProcedure> getProcedures(DBRProgressMonitor monitor, String name)
        throws DBException {
        return DBUtils.findObjects(getProcedures(monitor), name);
    }

    @Override
    public List<? extends AltibaseProcedure> getProceduresOnly(DBRProgressMonitor monitor) throws DBException {
        if (!dataSource.splitProceduresAndFunctions()) {
            return getProcedures(monitor);
        }
        List<AltibaseProcedure> filteredProcedures = new ArrayList<>();
        for (AltibaseProcedure proc : CommonUtils.safeList(getProcedures(monitor))) {
    ...
    public AltibaseSynonym getSynonym(DBRProgressMonitor monitor, String name) throws DBException {
    ...
    private synchronized void loadProcedures(DBRProgressMonitor monitor)
        ...
        if (packages != null) {
            for (AltibasePackage pack : packages) {
                pack.orderProcedures();
            }
        }
    }

    public void addProcedure(AltibaseProcedure procedure) {
        if (procedures == null) {
    ...
    public boolean hasProcedure(String name) {
        if (procedures != null) {
            for (AltibaseProcedure proc : procedures) {
    ...
    public void addPackage(AltibasePackage procedurePackage) {
        if (packages == null) {
    ...
    public class ContainerTriggerCache extends JDBCObjectCache<AltibaseStructContainer, AltibaseTrigger> {

        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull AltibaseStructContainer container) throws SQLException {
            return container.getDataSource().getMetaModel().prepareContainerTriggersLoadStatement(session, container);
        }

        @Nullable
        @Override
        protected AltibaseTrigger fetchObject(@NotNull JDBCSession session, @NotNull AltibaseStructContainer container, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
            return container.getDataSource().getMetaModel().createContainerTriggerImpl(container, resultSet);
        }
    }

    class GenericSequenceCache extends JDBCObjectCache<AltibaseObjectContainer, AltibaseSequence> {

        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull AltibaseObjectContainer container) throws SQLException {
            return container.getDataSource().getMetaModel().prepareSequencesLoadStatement(session, container);
        }

        @Nullable
        @Override
        protected AltibaseSequence fetchObject(@NotNull JDBCSession session, @NotNull AltibaseObjectContainer container, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
    ...
    class GenericSynonymCache extends JDBCObjectCache<AltibaseObjectContainer, AltibaseSynonym> {
        ...
        @Nullable
        @Override
        protected AltibaseSynonym fetchObject(@NotNull JDBCSession session, @NotNull AltibaseObjectContainer container, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
