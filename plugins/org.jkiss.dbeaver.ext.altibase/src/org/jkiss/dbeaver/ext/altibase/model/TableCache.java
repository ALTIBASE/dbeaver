...
public class TableCache extends JDBCStructLookupCache<GenericStructContainer, GenericTableBase, GenericTableColumn> {
    ...
    {
        super(GenericUtils.getColumn(dataSource, AltibaseGenericConstants.OBJECT_TABLE, JDBCConstants.TABLE_NAME));
        this.dataSource = dataSource;
        this.tableObject = dataSource.getMetaObject(AltibaseGenericConstants.OBJECT_TABLE);
        this.columnObject = dataSource.getMetaObject(AltibaseGenericConstants.OBJECT_TABLE_COLUMN);
        ...
        throws SQLException, DBException
    {
        ...
        if (!CommonUtils.isEmpty(typeName)) {
            ...
            if (typeName.toUpperCase(Locale.ENGLISH).endsWith(AltibaseGenericConstants.TYPE_MODIFIER_IDENTITY)) {
                autoIncrement = true;
                typeName = typeName.substring(0, typeName.length() - AltibaseGenericConstants.TYPE_MODIFIER_IDENTITY.length());
            }
