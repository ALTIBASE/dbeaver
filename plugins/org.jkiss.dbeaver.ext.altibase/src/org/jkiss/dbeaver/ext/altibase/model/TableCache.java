...
public class TableCache extends JDBCStructLookupCache<AltibaseStructContainer, AltibaseTableBase, AltibaseTableColumn> {
    ...
    {
        super(AltibaseUtils.getColumn(dataSource, GenericConstants.OBJECT_TABLE, JDBCConstants.TABLE_NAME));
    ...
    protected AltibaseTableColumn fetchChild(@NotNull JDBCSession session, @NotNull AltibaseStructContainer owner, @NotNull AltibaseTableBase table, @NotNull JDBCResultSet dbResult)
        throws SQLException, DBException
    {
        String columnName = AltibaseUtils.safeGetStringTrimmed(columnObject, dbResult, JDBCConstants.COLUMN_NAME);
        int valueType = AltibaseUtils.safeGetInt(columnObject, dbResult, JDBCConstants.DATA_TYPE);
        int sourceType = AltibaseUtils.safeGetInt(columnObject, dbResult, JDBCConstants.SOURCE_DATA_TYPE);
        String typeName = AltibaseUtils.safeGetStringTrimmed(columnObject, dbResult, JDBCConstants.TYPE_NAME);
        long columnSize = AltibaseUtils.safeGetLong(columnObject, dbResult, JDBCConstants.COLUMN_SIZE);
        boolean isNotNull = AltibaseUtils.safeGetInt(columnObject, dbResult, JDBCConstants.NULLABLE) == DatabaseMetaData.columnNoNulls;
        Integer scale = null;
        try {
            scale = AltibaseUtils.safeGetInteger(columnObject, dbResult, JDBCConstants.DECIMAL_DIGITS);
        } catch (Throwable e) {
        ...
        int radix = 10;
        try {
            radix = AltibaseUtils.safeGetInt(columnObject, dbResult, JDBCConstants.NUM_PREC_RADIX);
        } catch (Exception e) {
            log.warn("Error getting column radix", e);
        }
        String defaultValue = AltibaseUtils.safeGetString(columnObject, dbResult, JDBCConstants.COLUMN_DEF);
        String remarks = AltibaseUtils.safeGetString(columnObject, dbResult, JDBCConstants.REMARKS);
        long charLength = AltibaseUtils.safeGetLong(columnObject, dbResult, JDBCConstants.CHAR_OCTET_LENGTH);
        int ordinalPos = AltibaseUtils.safeGetInt(columnObject, dbResult, JDBCConstants.ORDINAL_POSITION);
