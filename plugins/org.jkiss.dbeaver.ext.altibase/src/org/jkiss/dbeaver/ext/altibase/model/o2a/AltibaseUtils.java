...
public class AltibaseUtils {
    ...
    public static String getSysCatalogHint(AltibaseDataSource datasource)
    {
    	return "";
    }
    ...
    public static String getDDL(
        ...
        String objectType,
        AltibaseTableBase object,
        AltibaseDDLFormat ddlFormat,
    ...
    private static String invokeDBMSMetadataGetDependentDDL(JDBCSession session, AltibaseSchema schema, AltibaseTableBase object, DBMSMetaDependentObjectType dependentObjectType) {
        String ddl = "";
        try (JDBCPreparedStatement dbStat = session.prepareStatement(
    ...
    private static String addCommentsToDDL(DBRProgressMonitor monitor, AltibaseTableBase object, String ddl) {
        ...
        try {
            List<AltibaseTableColumn> attributes = object.getAttributes(monitor);
            if (!CommonUtils.isEmpty(attributes)) {
                List<DBEPersistAction> actions = new ArrayList<>();
                if (CommonUtils.isEmpty(objectComment)) {
                    ddlBuilder.append("\n");
                }
                for (AltibaseTableColumn column : CommonUtils.safeCollection(attributes)) {
    ...
    {
        AltibaseSchema schema = object.getSchema();
        if (schema == null) {
            return;
        }
        ...
        AltibaseSchema defaultSchema = ((AltibaseExecutionContext)executionContext).getDefaultSchema();
    ...
    public static String getSysSchemaPrefix(AltibaseDataSource dataSource) {
        boolean useSysView = CommonUtils.toBoolean(dataSource.getContainer().getConnectionConfiguration().getProviderProperty(AltibaseConstants.PROP_METADATA_USE_SYS_SCHEMA));
        if (useSysView) {
            return AltibaseConstants.SCHEMA_SYS + ".";
        } else {
            return "";
        }
    }
    ...
    public static String getSource(DBRProgressMonitor monitor, AltibaseSourceObject sourceObject, boolean body, boolean insertCreateReplace) throws DBCException
    {
        ...
        final AltibaseSchema sourceOwner = sourceObject.getSchema();
        if (sourceOwner == null) {
            log.warn("No source owner for object '" + sourceObject.getName() + "'");
            return null;
        }
        monitor.beginTask("Load sources for '" + sourceObject.getName() + "'...", 1);
        String sysViewName = AltibaseConstants.VIEW_DBA_SOURCE;
        if (!sourceObject.getDataSource().isViewAvailable(monitor, AltibaseConstants.SCHEMA_SYS, sysViewName)) {
            sysViewName = AltibaseConstants.VIEW_ALL_SOURCE;
        }
    ...
    public static boolean getObjectStatus(
        ...
        AltibaseObjectType objectType)
        throws DBCException
    {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, object, "Refresh state of " + objectType.getTypeName() + " '" + object.getName() + "'")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT STATUS FROM " + AltibaseUtils.getAdminAllViewPrefix(monitor, object.getDataSource(), "OBJECTS") + " WHERE OBJECT_TYPE=? AND OWNER=? AND OBJECT_NAME=?")) {
    ...
    public static String getAdminAllViewPrefix(DBRProgressMonitor monitor, AltibaseDataSource dataSource, String viewName)
    {
        boolean useDBAView = CommonUtils.toBoolean(dataSource.getContainer().getConnectionConfiguration().getProviderProperty(AltibaseConstants.PROP_ALWAYS_USE_DBA_VIEWS));
        if (useDBAView) {
            String dbaView = "DBA_" + viewName;
            if (dataSource.isViewAvailable(monitor, AltibaseConstants.SCHEMA_SYS, dbaView)) {
                return AltibaseUtils.getSysSchemaPrefix(dataSource) + dbaView;
            }
        }
        return AltibaseUtils.getSysSchemaPrefix(dataSource) + "ALL_" + viewName;
    }
}
