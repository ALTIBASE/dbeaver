...
public class AltibaseTablespace extends AltibaseGlobalObject implements DBPRefreshableObject, DBPObjectStatistics
{
    ...
    public AltibaseDataFile getFile(DBRProgressMonitor monitor, long relativeFileNo) throws DBException
    {
    ...
    private void loadSizes(DBRProgressMonitor monitor) throws DBException {
        try (final JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load tablespace '" + getName() + "' statistics")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement("SELECT * FROM\n" +
                "(SELECT SUM(F.BYTES) AVAILABLE_SPACE FROM " + AltibaseUtils.getSysSchemaPrefix(getDataSource()) + "DBA_DATA_FILES F WHERE F.TABLESPACE_NAME=?) XDF,\n" +
                "(SELECT SUM(S.BYTES) USED_SPACE FROM " + AltibaseUtils.getSysSchemaPrefix(getDataSource()) + "DBA_SEGMENTS S WHERE S.TABLESPACE_NAME=?) XS")) {
    ...
    static class FileCache extends JDBCObjectCache<AltibaseTablespace, AltibaseDataFile> {
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull AltibaseTablespace owner) throws SQLException
        {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT * FROM " + AltibaseUtils.getSysSchemaPrefix(owner.getDataSource()) + "DBA_" +
        ...
        @Override
        protected AltibaseDataFile fetchObject(@NotNull JDBCSession session, @NotNull AltibaseTablespace owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException
        {
            return new AltibaseDataFile(owner, resultSet, owner.getContents() == Contents.TEMPORARY);
        }
    }
